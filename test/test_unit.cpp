...
AllWize - Automated Unit Test suite
...
void test_persist() {
    ...
    TEST_ASSERT_EQUAL(channel2, channel3);

    // factory reset
    TEST_ASSERT_TRUE(allwize->factoryReset());

    // get channel once more (factory channel is 3)
    uint8_t channel4 = allwize->getChannel();
    TEST_ASSERT_EQUAL((uint8_t) 3, channel4);
...
void setup() {
    ...
    if (!allwize->waitForReady()) {
        TEST_FAIL_MESSAGE("Error connecting to the radio module");
    }
