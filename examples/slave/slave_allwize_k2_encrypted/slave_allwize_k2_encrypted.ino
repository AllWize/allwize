...
The message is encrypted by the radio module.
...
#define WIZE_REGISTER_ID        1
#define WIZE_UID                0x20212223
#define WIZE_MID                0x4824
#define WIZE_VERSION            0x01
#define WIZE_DEVICE_TYPE        0x00
#define WIZE_KEY                { 0xc8, 0xc7, 0xa4, 0xe3, 0x5d, 0xe3, 0x78, 0x97, 0x0d, 0x52, 0x76, 0x4c, 0x14, 0x1b, 0x3c, 0xd7 }
...
void wizeSetup() {
    ...
    // This is a slave device (sensor, sender)
    allwize.slave();

    // Define the radio configuration (channel, power, data rate)
    ...
    allwize.setVersion(WIZE_VERSION);
    allwize.setDevice(WIZE_DEVICE_TYPE);

    // Set the encryption keys and enable encryption
    uint8_t key[] = WIZE_KEY;
    allwize.setKey(WIZE_REGISTER_ID, key);
    allwize.setWizeControl(WIZE_REGISTER_ID);

    //allwize.dump(DEBUG_SERIAL);

    String address = allwize.getAddress();
    DEBUG_SERIAL.print("[WIZE] Device address: 0x");
